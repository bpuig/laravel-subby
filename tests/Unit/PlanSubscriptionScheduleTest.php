...
use Bpuig\Subby\Jobs\PlanSubscriptionSchedule\SubscriptionScheduleProcessJob;
use Bpuig\Subby\Jobs\PlanSubscriptionSchedule\SubscriptionScheduleQueuerJob;
...
class PlanSubscriptionScheduleTest extends TestCase
{
    ...
    public function testSuccessfulJob()
    {
        ...
        $this->testUser->subscription('main')->toPlan($this->testPlanPro)->service('success')->timeout(200)->tries(1)->onDate($date)->setSchedule();

        $this->travelTo($date->add(5, 'second'));

        $pending = app(config('subby.schedule.models.plan_subscription_schedule'))
            ->pending()
            ->first();
    ...
    {
        $date = Carbon::now()->add(10, 'day');
        $this->testUser->subscription('main')->toPlan($this->testPlanPro)->service('fail')->timeout(200)->tries(1)->onDate($date)->setSchedule();
    ...
    public function testSubscriptionScheduleQueuerJob()
    {
        ...
        while ($i <= 10) {
            ...
            $this->testUser->subscription('main')->toPlan($plan)->service('success')->timeout(200)->tries(1)->onDate($date)->setSchedule();
            $i++;
        }
